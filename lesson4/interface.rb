class Interface
  ...
  def create_new_routes # создать новый маршрут
    ...
    @all_stations.each_with_index do |s, i| puts "#{i + 1}. #{s.show_name.capitalize}"
    end
  ...
  def add_new_stations # добавление станции в маршрут
    ...
    puts "And routes"
    @all_routes.each_with_index do |r, i| puts "#{i + 1}. Route from #{r.first.show_name.capitalize} to #{r.last.show_name.capitalize}"
    end
    puts "Choose station: "
    answer = gets.chomp.to_i
    puts "Choose route: "
  ...
  def remove_railway_wagon # отцепка вагонов от поезда
    @all_trains.each_with_index do |t, i| puts "#{i + 1}. #{t.type.to_s.capitalize} train, number #{t.number} with #{t.show_number_of_wagons} railway wagon"
    end
  ...
  def show_list # просмотр списка станций и список поездов
    ...
    @all_trains.each_with_index do |t, i| puts "\t#{i + 1}. #{t.type.to_s.capitalize} train, number #{t.number}, location - #{t.current_station.name.to_s}"
    end
  end
end